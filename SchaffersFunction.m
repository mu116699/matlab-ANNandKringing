% 此函数的测试结果表明Kriging_RBF函数具有很好的测试结果。
clc
clear
%定义采样的区间
Interval = 2;
...
samplevalue = 0.5+ (sin(sqrt(sampletotal(1,:).^2+sampletotal(2,:).^2))-0.5)...
...
test_y_true = 0.5+ (sin(sqrt(test_x_true(1,:).^2+test_x_true(2,:).^2))-0.5)...
    ./(1+0.001*(test_x_true(1,:).^2+test_x_true(2,:).^2));
