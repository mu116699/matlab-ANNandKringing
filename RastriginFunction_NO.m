% 使用这个函数可以看到最终的测试结果具有高度的一致性，
% 但是从结果来看还是kriging的测试结果具有更好的拟合结果
% 但是使用Kriging_RBF系统的建模会变差，但是和RBF神经网络的结果很相似
% 使用的数据点增加到500个测试的结果也是一样的，最终的测试结果也是很差的
clc
clear
%定义采样的区间
Interval = 1;
...
samplevalue = sampletotal(1,:).^2+sampletotal(2,:).^2-...
cos(18*sampletotal(1,:))-cos(18*sampletotal(2,:));
% 4*sampletotal(1,:).^2-2.1*sampletotal(1,:).^4+1/3*...
...
test_y_true = test_x_true(1,:).^2+test_x_true(2,:).^2-...
cos(18*test_x_true(1,:))-cos(18*test_x_true(2,:));
